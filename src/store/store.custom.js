...
const createCustomCategoryStore = (/** @type {CustomCategory []} */ allCustomCategories) => {
    ...
    const updateAllCategoriesFromSave = (allCategories) => {
        ...
        storageService.saveCategories(allCategories);
    };
    subscribe((all) => (customCategories = all));
    const getAll = () => customCategories;
    const deleteCategory = (catId) => updateCustomCategory(catId, {
        cardsIds: [],
        customId: catId,
        name: ""
    });
    /** @type {( category :CustomCategory )=> boolean} */
    const isCatExist = (category) => !!(category.name && category.cardsIds.length);
    return {
        subscribe,
...
const baseCategories = [1, 2, 3, 4].map((c) => ({
    cardsIds: [], name: "", customId: c,
}));

const lsCategories = storageService.getCategories();

export const categoryStore = createCustomCategoryStore(lsCategories || baseCategories);
